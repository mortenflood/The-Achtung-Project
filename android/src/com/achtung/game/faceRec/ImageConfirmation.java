...
public class ImageConfirmation extends Activity {
	...
	private ImageView homeButton; // ImageView to go to com.mygdx.game.MainMenu or login screen (main activity)
	private ImageView confirmationView; // ImageView to display the selected image
	public MainFaceRecActivity faceRecog;
	...
	private boolean mainFaceRecMenu = true;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int angle = extras
				.getInt("com.qualcomm.sdk.recognition.sample.com.mygdx.game.ImageConfirmation.orientation");
		boolean cameraFacingFront = extras
				.getBoolean("com.qualcomm.sdk.recognition.sample.com.mygdx.game.ImageConfirmation.switchCamera");
		boolean throughGallery = extras
		// If the image is coming through the gallery
				.getBoolean("com.qualcomm.sdk.recognition.sample.com.mygdx.game.ImageConfirmation.through.gallery");
		...
		faceObj = MainFaceRecActivity.faceObj;
		
		storedBitmap = BitmapFactory
				.decodeByteArray(data, 0, data.length, null);

		faceRecog = new MainFaceRecActivity();
			...
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				if (inputName != null && inputName.trim().length() != 0) {
					if (!hash.containsKey(inputName)) {
						...
						//Decide the behaviour of home button.
						mainMenu = false;
						mainFaceRecMenu = true;
					} else {
						Toast.makeText(getApplicationContext(),
